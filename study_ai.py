...
def generate_cheatsheet_data(
    ...
    {
      "front": {
        "words":   [{"en":..,"def_en":..,"def_kr":..,"kr":..}, ...],   # 대표 15개 (영영풀이+한글해석)
...
{{
  "front": {{
    "words": [{{"en":"단어","def_en":"영영풀이(쉬운 1줄 영어 정의)","def_kr":"영영풀이의 한글 해석","kr":"한글뜻"}}],
...
규칙:
- words: 학생들은 이미 단어를 외운 상태이므로 단순 뜻 나열은 금지. 그 단원의 **대표 핵심 단어 15개**만 선정
  (자료가 15개보다 적으면 있는 만큼). 최근 내신에 '영영풀이(English definition)→단어 맞히기' 유형이 자주 나오므로,
  그렇게 출제될 법한 단어 위주로 고를 것.
- def_en: 중학생이 이해할 수준의 **쉬운 영어 1줄 정의**. ★정의문 안에 그 단어(en) 자체나 어형을 절대 쓰지 말 것★ (그러면 답이 보임).
- def_kr: 그 def_en(영영풀이)을 자연스러운 한국어로 옮긴 해석 (시험 직전 빠르게 이해하도록 반드시 채울 것).
  kr: 단어 자체의 한글 뜻. tip 필드는 더 이상 쓰지 않음.
