...
- 1단계 ✅ 교육과정(학년·주제) → AI가 지문/대화문·단어 창작 (`study_ai.generate_curriculum_content`, app.py "새 노트 추가")
- 2단계 ✅ 생성물 노트 라이브러리 저장 → 모든 학습 기능 자동 연결 (save_note)
- 3단계 ✅ **생각 유도형(서술형 DNA) 자동 동봉** — 노트 저장 직후 그 콘텐츠로 `generate_essay_questions`→`question_bank`(source_type=essay) 자동 적재.
  app.py 저장 흐름 + 체크박스 `gc_with_essay`(기본 ON). AI 한도/카운트 연동. 만드는 순간 서술형 DNA에 문제 준비됨.

**남은 확장 후보(미착수)**: 전체유형 통합생성(단어+대화+본문 한번에) · 교육과정 문법포인트 지정 입력 · 생성 품질 재생성/검수 UI · OX·객관식 문제도 동봉.
